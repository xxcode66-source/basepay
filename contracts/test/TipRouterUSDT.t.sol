...
contract TipRouterUSDTTest is Test {
    ...
    function setUp() public {
        vm.prank(owner); usdt = new MockUSDT();
        router = new TipRouterUSDT(address(usdt), treasury, owner);
        usdt.mint(viewer, 100 * 10 ** 6);
    }

    function _tip(uint256 amount) internal {
        vm.prank(viewer); usdt.approve(address(router), amount);
        vm.prank(viewer); router.tip(streamer, amount, "Test tip");
    }

    function test_TipSplitsCorrectly() public { _tip(FIVE_USDT); assertEq(usdt.balanceOf(treasury), FEE_ON_FIVE); assertEq(usdt.balanceOf(streamer), FIVE_USDT - FEE_ON_FIVE); }
    function test_FeeScalesWithAmount() public { _tip(10_000_000); assertEq(usdt.balanceOf(treasury), 500_000); }
    function test_SmallTipStillPaysFee() public { _tip(5); assertEq(usdt.balanceOf(treasury), 1); assertEq(usdt.balanceOf(streamer), 4); }
    function test_RevertWhen_AmountIsZero() public {
        vm.prank(viewer); vm.expectRevert(TipRouterUSDT.AmountTooLow.selector);
        router.tip(streamer, 0, "");
    }
    function test_RevertWhen_SelfTip() public {
        vm.prank(viewer); usdt.approve(address(router), FIVE_USDT);
        vm.prank(viewer); vm.expectRevert(TipRouterUSDT.SelfTip.selector);
